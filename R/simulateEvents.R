...
#'
#'@param Data Data frame or matrix containing the data which will undergo the transformations.
#'@param Params Vector or vector of strings indicating the column names (in case Data is a data frame) or the column
...
#'Current valid options include sinusoidal, square, ramp, binomial and slowsinusoidal. If Params contains more that one
#'elements and Event_type only contains one element the same transformation will be applied to all given Params
#'@param Event_strength (Optional) Numeric Vector indicating the amplitude. Only valid for sinusoidal and square
#'transformations
#'@param Start_index Index indicated where the event should start
#'@param Event_duration Numeric, indicates the number of steps the transformation should last. Default is 100
#'@param Percentage Numeric value from 0 to 1. Alternative input indicating the percentage of data that
#'should be affected by the transformation. Either Event_duration or Percentage should be especified.
#'
#'@return Matrix or data frame containing the transformed columns
#'
#'@export
#'
#'@import imputeTS
...
#'@examples
#'
#'#Generate event of type sinusoidal and binomial on two columns of the stationBData data set
...
simulateEvents<-function(Data,
    ...
    cov_details<-cov(data_extracted, use="complete.obs")
    #sigma vector??
    sigma_vector<-sqrt(diag(cov_details))
                ...
                {#square case
                    ...
                    {#ramp case
                        ...
                        {#binomial case
                            ...
                            {#slowsinusoidal
                                data_extracted[Start_index+iter,Parloop]<-data_extracted[Start_index+iter,Parloop]+(sin((pi*iter)/500.0)*sigma_vector[Parloop])
                            }else{
